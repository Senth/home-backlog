...
- Then run the `/review` skill and ship only on a PASS. Cheap first, expensive
  once: `code-review` (diff, CLAUDE.md invariants) runs and is fixed *before*
  `browser-review` opens a browser at all — one session, one walk, acceptance
  and craft together, then the hostile checklist (console, offline, empty state,
  reload, `sv-SE`, volume). Fix loop caps at two rounds and re-runs scoped, not
  whole. `blocking` findings are never deferrable. The session that wrote the
  code never signs it off. Use `/review --code` when nothing user-visible
  changed — the browser pass is for user-visible surface, not for every file
  under `components/`.
- `.emulator-seed/` is the review fixture the emulators import (`yarn
  emulators:seed`). It is generated by the app — `yarn emulators:export`, never
  hand-written — and a feature that adds data worth reviewing against refreshes
  it in its cleanup phase.
