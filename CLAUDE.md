...
- Then run the `/review` skill and ship only on a PASS. It hands the change to
  three agents that did not write it — `code-review` (diff, CLAUDE.md
  invariants), `ux-review` (browser, craft plus the six personas) and
  `qa-review` (browser, acceptance plus offline / `sv-SE` / empty states) — and
  loops on the fixes. `blocking` findings are never deferrable. The session that
  wrote the code never signs it off.
