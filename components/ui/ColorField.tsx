...
interface ColorFieldProps {
	...
	value: string;
	/** Called with the normalized `#rrggbb` once the field holds a real color. */
	onChange: (color: string) => void;
}

/**
 * The color field without a platform picker (#100): a validated hex field,
 * until there is a native build to hang `<input type="color">`'s native
 * equivalent on.
 *
 * Every keystroke is only a draft; the parent hears the color the moment the
 * field holds one, normalized through the same `parseHex`/`toHex` pair the
 ...
 * color, the sentence beneath says so and nothing is written.
 */
export function ColorField({ label, value, onChange }: ColorFieldProps) {
	...
	// the field; a draft that is not a color yet never triggers this, because
