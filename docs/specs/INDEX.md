...
| [`boards-and-nodes`](boards-and-nodes.md) | The one document every board, project, task and subtask is made of, and every screen that renders it. The field set, the uniform-visibility invariant that makes privacy queryable, the two provably safe queries a board load merges, fractional `rank`, the column set frozen by depth, and board-ness derived from a stored child count rather than flagged. The board itself — drill-down, breadcrumbs, and a card menu — and the node detail screen carrying notes, due date, priority and effort, its steps, and notes that save themselves. | nodes, boards, columns, rank, privacy, indexes, detail, due dates |
