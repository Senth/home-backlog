...
`DELETE` honour `If-Match` against it, returning `412 version_mismatch` when it does not
agree and applying no precondition at all when the header is absent. It is the whole of the
concurrency control. An agent that read a card, thought about it and comes back to write
finds out that somebody edited it in between instead of silently overwriting them.
